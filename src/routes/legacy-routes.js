...
export async function setupLegacyRoutes(app) {
    const env = getEnv();
    app.options('*', async (c) => {
        addCorsHeaders(c);
        //@ts-expect-error
    ...
    // Associate routes
    app.get('/associates/download-pdf/:id', async (c) => {
        try {
    ...
    app.delete('/associate/deactivate', async (c) => {
        try {
            const body = await c.req.json();
